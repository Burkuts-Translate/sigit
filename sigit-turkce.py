#! /usr/bin/env python3
# Yazar @termuxhackers.id
# Çeviri @burkuts
# Facebook Sayfamı Takip Ederek Yazarı Destekle https://fb.me/termuxhackers.id
# İnstagram sayfamı Takip Ederek Çevirmeni Destekle https://instagram.com/burkut.sh
# Dikkat : Bu Yeniden Kodlanmış S.I.G.I.T Yazılımının Çeviri Yapılmış Hali'dir!
# Son Güncelleme: 21/04/2021 - versiyon 1.0
      ...
      <KKKKKKKKkr,                       ,rSKKKKKKKK<
       -"v]qj;-                             -;jq]v"-
           ...
           {d}Simple Information Gathering Toolkit{w}
               {d}Yazar {w}{r}@Termuxhackers.id{w}
               {d} Çeviren{w}{r}@burkuts{w}"""
...
def menu():
    ...
    print(f"""
        ...
        {w}{b}  01{w} Userrecon    {d} Kullanıcı Adı Bulma
        {w}{b}  02{w} Facedumper   {d} Facebook Bilgilerini Çekme
        {w}{b}  03{w} Mailfinder   {d} E-Posta Bulma
        {w}{b}  04{w} Godorker     {d} Google Dork Aratma
        {w}{b}  05{w} Phoneinfo    {d} Telefon Nuamarasın'dan Bilgi Çekme
        {w}{b}  06{w} DNSLookup    {d} Alan Adı sistem taraması
        {w}{b}  07{w} Whoislookup  {d} Alan Adının Kime Ait Olduğunu Ve Bilgilerini Bulun
        {w}{b}  08{w} Sublookup    {d} Alt Ağ Araması  ( Alt Alan Adını Bulma )
        {w}{b}  09{w} Hostfinder   {d} Ana Ağı bulun
        {w}{b}  10{w} DNSfinder    {d} Ana Ağın Sistemini Bulun
        {w}{b}  11{w} RIPlookup    {d} İP Araması
        {w}{b}  12{w} IPlocation   {d} İP Konumunu İzleme/Bulma
        """)
    mainmenu()
...
def iplocation():
    ...
    try: city = "ŞEHİR: "+req["city"]
    except KeyError: city = ""
    try: country = "ÜLKE: "+req["country"]
    except KeyError: country = ""
    try: loc = "LOKASYON: "+req["loc"]
    except KeyError: loc = ""
    try: org = "ORG: "+req["org"]
    except KeyError: org = ""
    try: tz = "ZAMAN DİLİMİ: "+req["timezone"]
    except KeyError: tz = ""
    z = [ip, city, country, loc, org, tz]
    for res in z:
        print(f"{space}{b}-{w} {res}")
    print(w+lines)
    getpass(space+"önceki menüye dönmek için enter tuşuna basınız ")
    menu()

def infoga(opt):
    x = str(input(f"{space}{b}>{w} Alan Adı Ve Ya İP Adresi Giriniz:{b} "))
    if not x: menu()
    ...
    print(w+lines)
    getpass(space+"önceki menüye sönmek için enter tuşuna basınız ")
    menu()
...
def godorker():
    dork = str(input(f"{space}{b}>{w} dork girin (inurl/intext/etc):{b} ").lower())
    if not dork: menu()
    print(w+lines)
    url = []
    ...
    for u in url:
        try:
            ...
            f.write(u+"\n")
            print(f"{space}{B} BULUNDU {w} {str(title)}\n{space}{d}{u}{w}")
    ...
    f.close()
    print(w+lines)
    print(f"{space}{b}>{w} {str(len(url))} şuraya kayıt edildi: {y}result_godorker.txt{w}")
    getpass(space+"önceki menüye dönmek için enter tuşuna basınız ")
    menu()
    
def mailfinder():
    fullname = str(input(f"{space}{b}>{w} bir e-mail adresi giriniz:{b} ").lower())
    ...
    f.close()
    print(w+lines)
    print(f"{space}{b}>{w} {str(len(ok))}  şuraya kayıt edildi: {y}result_mailfinder.txt{w}")
    getpass(space+"önceki menüye dönmek için enter tuşuna basınız ")
    menu()

def userrecon():
    username = str(input(f"{space}{w}{b}>{w} kullanıcı adı giriniz:{b} ").lower())
...
class Facebook():
    ...
    def facedumper(self):
        try:
            ...
            while True:
                coki = getpass(f"{space}{b}>{w} facebook cookies giriniz (gizli girdi): ")
                if coki: break
                else: continue
        cookies = {"cookie":coki}
        ...
        if "mbasic_logout_button" in str(req):
            ...
            exit(r+"* geçersiz kimlik bilgileri: cookies"+w)
            time.sleep(3)
            menu()
        print(f"""
        {w}{b}  01{w} Hepsini Çek     {d} Arkadaş listesindeki bütün bilgileri çek
        {w}{b}  02{w} Uid Bilgisini Çek     {d} Dump user id from friendlist
        {w}{b}  03{w} Email Bilgisini Çek   {d} Arkadaş Listesindeki Kullanıcıların Email Bilgisini Çek
        {w}{b}  04{w} Telefon Bilgisini Çek   {d} Arkadaş Listesindeki Kullanıcıların Teledon Bilgisini Çek
        {w}{b}  05{w} Doğum Günü Bilgisini Çen{d} Arkadaş Listesindeki Kullanıcıların Doğum Günü Bilgisini Çek
        {w}{b}  06{w} Lokasyon Bilgisini Çek{d} Arkadaş Listesindeki Kullanıcıların Lokasyon Bilgisini Çek
        """)
        while True:
    ...
    def dump_all(self):
        ...
        for data in res["data"]:
            try:
                ...
                try:
                    phone = RES["phone"]
                    print(f"{space}{b}-{w} Telefon: {phone}")
                except KeyError: pass
                try:
                    birthday = RES["birthday"]
                    print(f"{space}{b}-{w} Doğum Günü: {birthday}")
                except KeyError: pass
                try:
                    location = RES["location"]["name"]
                    print(f"{space}{b}-{w} Lokasyon: {location}")
                except KeyError: pass
            except KeyboardInterrupt: break
        print(w+lines)
        getpass(space+"önceki menüye dönmek için enter tuşuna tıklayınız ")
        menu()
    ...
    def dump_id(self):
        ...
        for data in res["data"]:
            try:
                id = data["id"]
                name = data["name"]
                print(f"{space}{B} BİTTİ {w} ID: {id} {r}->{w} {name}")
        ...
        f.close()
        print(w+lines)
        print(f"{space}{b}>{w} {str(len(listid))} şuraya kayıt edildi: {y}dump_idfriends.txt{w}")
        getpass(space+"önceki menüye dönmek için enter tuşuna tıklayınız ")
        menu()
    ...
    def dump_email(self):
        ...
        for data in res["data"]:
            try:
                ...
                try:
                    name = RES["name"]
                    email = RES["email"]
                    print(f"{space}{B} BİTTİ {w} Email: {email} {r}->{w} {name}")
        ...
        f.close()
        print(w+lines)
        print(f"{space}{b}>{w} {str(len(listmail))} şuraya kayıt edildi: {y}dump_email.txt{w}")
        getpass(space+"önceki menüye dönmek için enter tuşuna basınız ")
        menu()
    ...
    def dump_phone(self):
        ...
        for data in res["data"]:
            try:
                ...
                try:
                    ...
                    print(f"{space}{B} BİTTİ {w} TELEFON: {phone} {r}->{w} {name}")
        ...
        f.close()
        print(w+lines)
        print(f"{space}{b}>{w} {str(len(listphone))} şuraya kayıt edildi: {y}dump_phone.txt{w}")
        getpass(space+"önceki menüye dönmek için enter tuşuna basınız ")
        menu()
    ...
    def dump_birthday(self):
        ...
        for data in res["data"]:
            try:
                ...
                try:
                    name = RES["name"]
                    day = RES["birthday"]
                    print(f"{space}{B} BİTTİ {w} Doğum Günü: {day} {r}->{w} {name}")
                    listday.append(day)
        ...
        f.close()
        print(w+lines)
        print(f"{space}{b}>{w} {str(len(listday))} Şuraya kayıt edildi: {y}dump_birthday.txt{w}")
        getpass(space+"önceki menüye dönmek için enter tuşuna basınız ")
        menu()
        ...
        for data in res["data"]:
            try:
                ...
                try:
                    ...
                    listloc.append(loc)
                    print(f"{space}{B} BİTTİ {w} Lokasyon: {loc} {r}->{w} {name}")
                except KeyError: pass
            except KeyboardInterrupt:
                break
        f.close()
        print(w+lines)
        print(f"{space}{b}>{w} {str(len(listloc))} Şuraya kayıt edildi: {y}dump_location.txt{w}")
        getpass(space+"önceki menuye dönmek için enter tuşuna basınız ")
        menu()
